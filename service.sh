...
dev_build() {
  ...
  javac -d ./build/classes/ ./src/busroute/*.java
  if [ ! -d "./dist" ]; then
    mkdir ./dist
  fi
  cd ./build/classes
  jar cfe ../../dist/busroute.jar busroute.BusRouteServer busroute/*.class
  true
}
...
_run_smoke() {
  ...
  echo "Running smoke tests on $baseUrl..." && \
    ...
    (curl -fsS "$baseUrl/api/direct?dep_sid=5&arr_sid=19" | grep -E 'true|false')
}
